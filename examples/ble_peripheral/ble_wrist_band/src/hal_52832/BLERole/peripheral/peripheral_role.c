...
#if 1//(BLUETOOTH_DEV_ROLE == PERIPHERAL_MODE)
...
static ble_uuid_t m_adv_uuids[] =                                               /**< Universally unique service identifiers. */
{
    ...
    //{BLE_UUID_NUS_SERVICE, NUS_SERVICE_UUID_TYPE}  //not supported
};
...
#ifdef __SIMPLE_ACC
    ble_conn_params_change_conn_params(m_conn_handle, (ble_gap_conn_params_t *)&fast_conn_params);
#else
    ble_conn_params_change_conn_params(m_conn_handle, (ble_gap_conn_params_t *)&slow_conn_param);
#endif
    m_is_fast_conn_mode = false;
    NRF_LOG_DEBUG("[BLE]: slow connection\r\n");
}
void ble_fast_connection(void)
{
    ...
    if (m_is_fast_conn_mode == false)
    {
        ...
        ble_conn_params_change_conn_params(m_conn_handle, (ble_gap_conn_params_t *)&fast_conn_params);
        NRF_LOG_DEBUG("[BLE]: fast connection\r\n");
    }
}
...
//
//    nus_init.data_handler = wb_recive_data_handler;
//
...
//    APP_ERROR_CHECK(err_code);
    ...
    else
    {
      ...
      adv_params.interval        = APP_ADV_INERVAL_CONNECTED;
    }
